...
def response(msg):
    ...
    if ints[0]['intent'] == 'product_info':
        ...
        if menudata:
            if menudata[7] == 1:
                res = f" Food : {menudata[1]}\n, Price: {menudata[4]}\n, Description: {menudata[2]}\n, Ingredients : {menudata[5]}\n, Available"
            else:
                res = f" Food : {menudata[1]}\n, Price: {menudata[4]}\n, Description: {menudata[2]}\n, Ingredients : {menudata[5]}\n, Not Available {menudata[7]}"
        else:
            res = "Order not found."
    ...
    if ints[0]['intent'] == 'track.order':
        ...
        if orderdata:
            res = f"Order ID: {orderdata[0]}\n, Customer name : {orderdata[1]}\n, Orders: {orderdata[2]}\n, Delivery Status : {orderdata[5]}\n"
        else:
            res = "Order not found. Please enter an order ID"
